...
if (isset($data["blg"])) {
    ...
    // var_dump($data['blg']); // Check if the blog post data is returned correctly
...
// if (isset($data["blg"]) && is_array($data["blg"])) {
//     echo "<h1 class='my-2 detail-title'>" . htmlspecialchars($data["blg"]["title"]) . "</h1>";
//     echo "<img class='img-fluid rounded mx-auto d-block img-detail' src='data:image/jpeg;base64," . base64_encode($data["blg"]["photo"]) . "' alt='Blog Photo' />";
//     echo "<p class='detail-content mt-4'>" . nl2br(htmlspecialchars($data["blg"]["content"])) . "</p>";
// } else {
//     echo "Blog post not found or data is invalid.";
// }
